...
public class TournamentSetup 
{
	...
	public static void setupSchedule(Scanner keyboard)
	{
		try(Essentials essentials = new Essentials(new Database(Database.DatabaseType.PRODUCTION), null, null, null, null))
		{
			...
			TournamentSolution solvedTournament = TournamentSolver.solve(School.getSchools(essentials), Tournament.GAME_PER_SCHOOL / (Tournament.SCHOOLS_PER_TEAM * 2), Tournament.SCHOOLS_PER_TEAM * 2, "com/backend/models/optaplanner/TournamentSolverConfig.xml");
